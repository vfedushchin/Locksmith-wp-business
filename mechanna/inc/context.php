...
function mechanna_utility() {
	return mechanna_theme()->get_core()->modules['cherry-utility'];
}
 ...
 * @return void
 */
function mechanna_primary_content_class( $classes = array() ) {
	echo mechanna_get_layout_classes( 'content', $classes );
}
 ...
 * @return void
 */
function mechanna_sidebar_class( $classes = array() ) {
	echo mechanna_get_layout_classes( 'sidebar', $classes );
}
 ...
 * @return string
 */
function mechanna_get_layout_classes( $layout = 'content', $classes = array() ) {
	...
	$layout_classes = ! empty( mechanna_theme()->layout[ $sidebar_position ][ $sidebar_width ][ $layout ] ) ? mechanna_theme()->layout[ $sidebar_position ][ $sidebar_width ][ $layout ] : array();
	...
	$layout_classes = apply_filters( "mechanna_{$layout}_classes", $layout_classes );
 ...
 * @return string|void
 */
function mechanna_posts_list_class( $classes = array(), $echo = true ) {
	$layout_type      = get_theme_mod( 'blog_layout_type', mechanna_theme()->customizer->get_default( 'blog_layout_type' ) );
	$layout_type      = ! is_search() ? $layout_type : 'default';
	$sidebar_position = get_theme_mod( 'sidebar_position', mechanna_theme()->customizer->get_default( 'sidebar_position' ) );
	...
	foreach ( $sidebars as $sidebar ) {
		if ( mechanna_widget_area()->is_active_sidebar( $sidebar ) ) {
			$has_sidebars = true;
		}
	}
	...
	$classes = apply_filters( 'mechanna_posts_list_class', $classes );
