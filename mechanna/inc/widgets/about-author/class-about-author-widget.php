 ...
 * @package Mechanna
 */
if ( ! class_exists( 'Mechanna_About_Author_Widget' ) ) {

	class Mechanna_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'mechanna widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'mechanna' );
			$this->widget_id          = 'mechanna_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'mechanna' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'mechanna' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'mechanna' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'mechanna' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'mechanna' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'mechanna' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'mechanna' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'mechanna' ),
					'label' => esc_html__( 'Link label', 'mechanna' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => __( 'Select a user', 'mechanna' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'mechanna-author-avatar' );
...
add_action( 'widgets_init', 'mechanna_register_about_author_widgets' );
function mechanna_register_about_author_widgets() {
	register_widget( 'Mechanna_About_Author_Widget' );
}
