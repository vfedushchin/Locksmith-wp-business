...
if ( ! class_exists( 'Mechanna_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Mechanna_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name			= esc_html__( 'Taxonomy Tiles', 'mechanna' );
			$this->widget_description	= esc_html__( 'This widget displays images from taxonomy.', 'mechanna' );
			$this->widget_id			= 'mechanna-widget-taxonomy-tiles';
			...
			$this->settings				= array(
				...
				'category'=> array(
					...
					'label'				=> esc_html__( 'Select category to show', 'mechanna' ),
				...
				'description_length' => array(
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Description words length ( Set 0 to hide description. )', 'mechanna' ),
				),
				'show_post_count' => array(
					...
					'options'		=> array(
						'show_post_count_check' => esc_html__( 'Show post count', 'mechanna' ),
					),
				),
				'layout_type' => array(
					'type'				=> 'radio',
					...
					'options'			=> array(
						'grid' => array(
							'label'		=> esc_html__( 'Grid', 'mechanna' ),
							'slave'		=> 'layout_type_grid',
						),
						'tiles' => array(
							'label'		=> esc_html__( 'Tiles', 'mechanna' ),
						),
					),
					'label'				=> esc_html__( 'Choose Layout Type', 'mechanna' ),
				),
				'columns_number' => array(
					...
					'min_value'			=> '1',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Columns number ( layout type grid )', 'mechanna' ),
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				echo apply_filters( 'mechanna_taxonomy_tiles_widget_before', '<div class="row grid ' . $layout_type . '-columns columns-number-' . $columns_number . '" style="margin: 0 0 0 -' . $items_padding . 'px" >', $instance );
					...
					if ( $view_dir ){
						...
						$count = $this->utility->meta_data->get_post_count_in_term(
							array(
								...
								'sufix'			=> _n_noop( '%s post', '%s posts', 'mechanna' ),
				...
				echo apply_filters( 'mechanna_taxonomy_tiles_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'mechanna_register_taxonomy_tiles_widget' );
	function mechanna_register_taxonomy_tiles_widget() {
		register_widget( 'Mechanna_Taxonomy_Tiles_Widget' );
	}
