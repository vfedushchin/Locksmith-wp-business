...
add_filter( 'widget_text', '__tm_render_widget_macros' );
...
function __tm_modify_comment_form( $args ) {
	...
	$args['comment_field'] = '<p class="comment-form-comment"><textarea id="comment" class="comment-form__field" name="comment" placeholder="' . esc_html__( 'Comment', '__tm' ) . '" cols="45" rows="8" aria-required="true" required="required"></textarea></p>';
...
function __tm_extra_body_classes( $classes ) {
	...
	$header_layout      = get_theme_mod( 'page_layout_type', __tm_theme()->customizer->get_default( 'header_container_type' ) );
 ...
 * @return string
 */
function __tm_render_widget_macros( $text ) {
...
function bedentist_custom_sizes( $sizes ) {
	return array_merge( $sizes, array(
		'__tm-thumb-m' => 'custom',
		'__tm-post-thumbnail-large' => 'custom-lg'
	) );
}
