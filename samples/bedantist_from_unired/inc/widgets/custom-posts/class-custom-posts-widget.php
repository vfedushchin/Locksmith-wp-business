...
class __Tm_Custom_Posts_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_id = apply_filters( '__tm_custom_posts_widget_ID', 'widget-custom-postson' );
		$this->widget_cssclass = apply_filters( '__tm_custom_posts_widget_cssclass', 'widget-custom-postson' );
	...
	public function cut_text( $text, $length ) {
		return ( '0' !== $length ) ? wp_trim_words( $text, $length, esc_html__( ' ...', '__tm' ) ) : '' ;
	}
	...
	public function get_post_date( $visible = 'true' ) {
		if ( $visible === 'true') {
			...
			$date_icon = apply_filters( '__tm_custom_posts_date_icon', '<i class="material-icons">schedule</i>' );
			$permalink = $this->get_post_permalink();
	...
	public function get_the_author( $user_id, $visible = 'true' ) {
		if ( $visible === 'true' ) {
			...
			$html= '<div class="post-author">' . esc_html__( 'by ', '__tm' ) . $author . '</div>';
	...
	public function get_comment_count( $count , $visible = 'true' ) {
		return ( $visible === 'true' ) ? '<div class="post_comments"><i class="material-icons">chat_bubble_outline</i>' . $count . '</div>' : '' ;
	}
	 ...
	 * @return string
	 */
	public function get_post_image( $post, $image_size = '__tm-thumb-m', $image_mobile_size = '__tm-thumb-s' ) {
		...
		if( $post_thumbnail_id ){
			...
			$placeholder_attr = apply_filters( '__tm_custom_posts_placeholder_default_args',
				array(
					'width'			=> $size['width'],
					'height'		=> $size['height'],
					'background'	=> get_theme_mod( 'regular_accent_color_1', __tm_theme()->customizer->get_default( 'regular_accent_color_1' ) ),
					'foreground'	=> get_theme_mod( 'regular_accent_color_2', __tm_theme()->customizer->get_default( 'regular_accent_color_2' ) ),
	...
	public function get_button( $text = '', $visible = 'true' ){
		if ( $visible === 'true' ) {
			$permalink = $this->get_post_permalink();
			$icon = apply_filters( '__tm_custom_posts_read_more_icon', '' ); //<span class="btn__icon"></span>
	...
	public function widget( $args, $instance ) {
		...
		if ( $posts ) {
			...
			$image_size = apply_filters( '__tm_playlist_thumbnail_image_size', '__tm-thumb-m' );
			$image_mobile_size = apply_filters( '__tm_playlist_thumbnail_size_mobile', '__tm-thumb-s' );
