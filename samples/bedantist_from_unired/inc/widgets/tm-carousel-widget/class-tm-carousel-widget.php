...
class __Tm_Carousel_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_name        = esc_html__( 'Carousel Widget', '__tm' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
	...
	public function widget( $args, $instance ) {
		...
		$footer_widget_columns = get_theme_mod( 'footer_widget_columns', __tm_theme()->customizer->get_default( 'footer_widget_columns' ) );
	...
	public function get_carousel_loop( $posts_query ) {
		$image_size = apply_filters( '__tm_carousel_image_size',
			'__tm-thumb-560-350',
			$this->instance,
			$this->args
		);
		...
		while ( $posts_query->have_posts() ) : $posts_query->the_post();
			...
			$placeholder_args = apply_filters( '__tm_carousel_placeholder_args', array(
				'width'      => 560,
				'height'     => 350,
				'background' => '000',
				'foreground' => 'fff',
	...
	public function get_image( $id, $size, $placeholder_attr, $only_url = false ) {
		...
		if ( has_post_thumbnail( $id ) ) {
			...
			$default_placeholder_attr = apply_filters( '__tm_carousel_placeholder_default_args', array(
				'width'      => 900,
				'height'     => 500,
				'background' => '000',
				'foreground' => 'fff',
	...
	public function get_post_date( $post_id ) {
		 ...
		 * @var string
		 */
		$date_format = apply_filters( '__tm_carousel_post_dateformat', 'M d, Y', $this->instance );
