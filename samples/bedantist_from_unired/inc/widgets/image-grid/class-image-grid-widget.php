	...
	class __Tm_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function get_post_image( $post, $image_size ) {
			...
			if ( ! $image ) {
				...
				$placeholder_attr = apply_filters( '__tm_image_grid_widget_placeholder_default_args', array(
					'width'			=> $size['width'],
					'height'		=> $size['height'],
					'background'	=> '000',
					'foreground'	=> 'fff',
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $posts ) && $posts ) {
				...
				echo apply_filters( '__tm_image_grid_widget_before', '<div class="row columns-number-' . $columns_number . '" ' . $row_inline_style . '>', $instance );

				$image_size = apply_filters( '__tm_image_grid_widget_size', '__tm-thumb-270', $instance );
