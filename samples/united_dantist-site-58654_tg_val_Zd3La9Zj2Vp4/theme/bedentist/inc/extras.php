 ...
 * @return string
 */
function bedentist_get_template_part_name(){
 ...
 * @return string
 */
function bedentist_set_post_meta_value( $value ) {



	$queried_obj = apply_filters( 'bedentist_queried_object_id', false );
	...
	$curr_opions = 'bedentist_' . str_replace( 'theme_mod_', '', current_filter() );
 ...
 * @return string
 */
function bedentist_set_sidebar_position( $value ) {
	$sp = bedentist_set_post_meta_value( $value );
 ...
 * Sidebar position
 */
add_filter( 'theme_mod_sidebar_position', 'bedentist_set_sidebar_position' );
/**
 * Header container type
 */
add_filter( 'theme_mod_header_container_type', 'bedentist_set_post_meta_value' );
/**
* Content container type
*/
add_filter( 'theme_mod_content_container_type', 'bedentist_set_post_meta_value' );
/**
* Footer container type
*/
add_filter( 'theme_mod_footer_container_type', 'bedentist_set_post_meta_value' );
 ...
 * @return string
 */
function bedentist_render_macros( $string ) {

	$macros = apply_filters( 'bedentist_data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function bedentist_render_icons( $content ) {

	$icons     = bedentist_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'bedentist_render_icons_callback', $content );
}
 ...
 * @return string
 */
function bedentist_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function bedentist_get_render_icons_set() {
	return apply_filters( 'bedentist_render_icons_set', array(
 ...
 * @return string
 */
function bedentist_render_theme_url( $url ) {
 ...
 * @return string
 */
function bedentist_get_thumbnail_size() {
	$layout = get_theme_mod( 'blog_layout_type', bedentist_theme()->customizer->get_default( 'blog_layout_type' ) );
 ...
 */
function bedentist_get_image_id_by_url( $image_src ) {
	global $wpdb;
...
function bedentist_post_formats_gallery() {
	if ( ! in_array( get_theme_mod( 'blog_layout_type' ), array( 'masonry-2-cols', 'masonry-3-cols' ) ) ) {
		return do_action( 'cherry_post_format_gallery', array(
			'size' => bedentist_get_thumbnail_size(),
		) );
	}

	$images = bedentist_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images(false);
