...
class __Tm_Instagram_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display a list of photos from Instagram network.', 'bedentist' );
		$this->widget_id          = 'bedentist_widget_instagram';
		$this->widget_name        = esc_html__( 'Instagram', 'bedentist' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'value' => esc_html__( 'Instagram', 'bedentist' ),
				'label' => esc_html__( 'Title', 'bedentist' ),
			),
			'client_id' => array(
				'type'  => 'text',
				'value' => '',
				'label' => esc_html__( 'Client ID', 'bedentist' ),
			),
			'tag' => array(
				'type'  => 'text',
				'value' => '',
				'label' => esc_html__( 'Hashtag (enter without `#` symbol)', 'bedentist' ),
			),
			'image_counter' => array(
				'type'       => 'stepper',
				'value'      => '6',
				...
				'min_value'  => '1',
				'step_value' => '1',
				'label'      => esc_html__( 'Number of photos', 'bedentist' ),
			),
			'display_caption' => array(
				...
				'options' => array(
					'display_caption_check' => esc_html__( 'Caption', 'bedentist' ),
				),
			),
			'display_date' => array(
				'type'  => 'checkbox',
				...
				'options' => array(
					'display_date_check' => esc_html__( 'Date', 'bedentist' ),
				),
			),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( empty( $instance['client_id'] ) ) {
			return print $args['before_widget'] . esc_html__( 'Please, enter your Instagram CLIENT ID.', 'bedentist' ) . $args['after_widget'];
		}

		if ( empty( $instance['tag'] ) ) {
			return print $args['before_widget'] . esc_html__( 'Please, enter #hashtag.', 'bedentist' ) . $args['after_widget'];
		}
		...
		if ( ! $photos ) {
			return print $args['before_widget'] . esc_html__( 'No photos. Maybe you entered a invalid CLIENT ID or hashtag.', 'bedentist' ) . $args['after_widget'];
		}
		...
		printf( '<div class="%s">',
			join( ' ', apply_filters( 'bedentist_instagram_widget_wrapper_class', array( 'instagram__items' ) ) )
		);
		...
		foreach ( $result['data'] as $photo ) {
			...
			if ( in_array( 'caption', $config ) ) {
				...
				$_photo['caption'] = substr(
					$caption,
					0,
					apply_filters( 'bedentist_instagram_widget_caption_length', 10 )
				);
			}
	...
	public function get_image( $photo ) {
		...
		return sprintf( '<a class="instagram__link" href="%s" target="_BeDENTIST" rel="nofollow"><img class="instagram__img" src="%s" alt="" width="%s" height="%s"><span class="instagram__cover"></span></a>', esc_url( $photo['link'] ), esc_url( $photo['image'] ), $width, $height );
	}
...
add_action( 'widgets_init', 'bedentist_register_instagram_widget' );
function bedentist_register_instagram_widget() {
