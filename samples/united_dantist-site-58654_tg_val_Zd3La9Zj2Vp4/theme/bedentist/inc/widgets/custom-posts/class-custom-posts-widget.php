...
class __Tm_Custom_Posts_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		$this->widget_name = esc_html__( 'Custom Posts', 'bedentist' );
		$this->widget_description = esc_html__( 'Display custom posts your site.', 'bedentist' );
		$this->widget_id = apply_filters( 'bedentist_custom_posts_widget_ID', 'widget-custom-postson' );
		$this->widget_cssclass = apply_filters( 'bedentist_custom_posts_widget_cssclass', 'widget-custom-postson' );

		$this->settings = array(
			'title' => array(
				'type'				=> 'text',
				'value'				=> esc_html__( 'Custom Posts', 'bedentist' ),
			...
			'terms_type' => array(
				...
				'options'			=> array(
					...
					'post_format'		=> array(
						'label'		=> esc_html__( 'Post Format', 'bedentist' ),
			...
			'post_format' => array(
				...
				'options'			=> false,
				'label'				=> esc_html__( 'Select post format', 'bedentist' ),
				'multiple'			=> true,
				'placeholder'		=> esc_html__( 'Select post format', 'bedentist' ),
			...
			'posts_per_page' => array(
				...
				'max_value'			=> 50,
				'min_value'			=> 0,
				'label'				=> esc_html__( 'Posts count ( Set 0 to show all. )', 'bedentist' ),
			),
			'post_offset' => array(
				'type'				=> 'stepper',
				'value'				=> '0',
				...
				'min_value'			=> '0',
				'step_value'		=> '1',
				'label'				=> esc_html__( 'Offset post', 'bedentist' ),
			),
			'title_length' => array(
				'type'				=> 'stepper',
				'value'				=> '10',
				...
				'min_value'			=> '0',
				'step_value'		=> '1',
				'label'				=> esc_html__( 'Title words length ( Set 0 to hide title. )', 'bedentist' ),
			),
			'excerpt_length' => array(
				...
				'min_value'			=> '0',
				'step_value'		=> '1',
				'label'				=> esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'bedentist' ),
			),
			'mate_data' => array(
				'type'				=> 'checkbox',
				'value'				=> array(
				...
				'options'				=> array(
					'date'				=> esc_html__( 'Date', 'bedentist' ),
					'author'			=> esc_html__( 'Author', 'bedentist' ),
					'comment_count'		=> esc_html__( 'Comment count', 'bedentist' ),
					'category'			=> esc_html__( 'Category', 'bedentist' ),
					'post_tag'			=> esc_html__( 'Tag', 'bedentist' ),
					'more_button'		=> esc_html__( 'More Button', 'bedentist' ),
				),
				'label'				=> esc_html__( 'Display post meta data', 'bedentist' ),
			),
			'button_text' => array(
				'type'				=> 'text',
				'value'				=> 'Read More',
				'label'				=> esc_html__( 'Post read more button label', 'bedentist' ),
			),
		);
	...
	public function cut_text( $text, $length ) {
		return ( '0' !== $length ) ? wp_trim_words( $text, $length, esc_html__( ' ...', 'bedentist' ) ) : '' ;
	}
	...
	public function get_post_date( $visible = 'true' ) {
		if ( $visible === 'true') {
			...
			$date_icon = apply_filters( 'bedentist_custom_posts_date_icon', '' );
			$permalink = $this->get_post_permalink();
	...
	public function get_the_author( $user_id, $visible = 'true' ) {
		if ( $visible === 'true' ) {
			...
			$html= '<div class="post-author">' . esc_html__( 'by ', 'bedentist' ) . $author . '</div>';
	 ...
	 * @return string
	 */
	public function get_post_image( $post, $image_size = 'bedentist-thumb-m', $image_mobile_size = 'bedentist-thumb-s' ) {
		...
		if( $post_thumbnail_id ){
			...
			$placeholder_attr = apply_filters( 'bedentist_custom_posts_placeholder_default_args',
				array(
					'width'			=> $size['width'],
					'height'		=> $size['height'],
					'background'	=> get_theme_mod( 'regular_accent_color_1', bedentist_theme()->customizer->get_default( 'regular_accent_color_1' ) ),
					'foreground'	=> get_theme_mod( 'regular_accent_color_2', bedentist_theme()->customizer->get_default( 'regular_accent_color_2' ) ),
	...
	public function get_button( $text = '', $visible = 'true' ){
		if ( $visible === 'true' ) {
			$permalink = $this->get_post_permalink();
			$icon = apply_filters( 'bedentist_custom_posts_read_more_icon', '' ); //<span class="btn__icon"></span>
	...
	public function widget( $args, $instance ) {
		...
		if ( $posts ) {
			...
			$image_size = apply_filters( 'bedentist_playlist_thumbnail_image_size', 'bedentist-thumb-m' );
			$image_mobile_size = apply_filters( 'bedentist_playlist_thumbnail_size_mobile', 'bedentist-thumb-s' );
...
add_action( 'widgets_init', 'bedentist_register_custom_posts_widget' );
function bedentist_register_custom_posts_widget() {
