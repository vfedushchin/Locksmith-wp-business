...
class __Tm_Subscribe_Follow_Widget extends Cherry_Abstract_Widget
{
    ...
    public function __construct()
    {
        ...
        $this->widget_cssclass = 'bedentist widget-subscribe';
        $this->widget_description = esc_html__('Display subscribe form and follow links.', 'bedentist');
        $this->widget_id = 'bedentist_widget_subscribe_follow';
        $this->widget_name = esc_html__('Subscribe and Follow', 'bedentist');
        $this->settings = array(
            'enable_subscribe' => array(
                ...
                'options' => array(
                    'enable_subscribe' => esc_html__('Enable Subscribe Box', 'bedentist'),
                ),
            ),
            'subscribe_title' => array(
                'type' => 'text',
                'value' => esc_html__('Subscribe', 'bedentist'),
                'label' => esc_html__('Subscribe Title', 'bedentist'),
            ),
            'subscribe_message' => array(
                'type' => 'textarea',
                'label' => esc_html__('Subscribe text message', 'bedentist'),
            ),
            'subscribe_input' => array(
                'type' => 'text',
                'value' => esc_html__('Enter your email', 'bedentist'),
                'label' => esc_html__('Subscribe input placeholder', 'bedentist'),
            ),
            'subscribe_submit' => array(
                'type' => 'text',
                'value' => esc_html__('Subscribe', 'bedentist'),
                'label' => esc_html__('Subscribe submit label', 'bedentist'),
            ),
            'subscribe_success' => array(
                'type' => 'text',
                'value' => esc_html__('You successfully subscribed', 'bedentist'),
                'label' => esc_html__('Subscribe success', 'bedentist'),
            ),
            'enable_follow' => array(
                'type' => 'checkbox',
                'value' => array(
                ...
                'options' => array(
                    'enable_follow' => esc_html__('Enable Follow Box', 'bedentist'),
                ),
            ),
            'follow_title' => array(
                'type' => 'text',
                'value' => esc_html__('Follow', 'bedentist'),
                'label' => esc_html__('Follow Title', 'bedentist'),
            ),
            'follow_message' => array(
                'type' => 'textarea',
                'label' => esc_html__('Follow text message', 'bedentist'),
            ),
        );

        add_action('wp_ajax_bedentist_subscribe', array($this, 'process_subscribe'));
        add_action('wp_ajax_nopriv_bedentist_subscribe', array($this, 'process_subscribe'));

        $this->errors = array(
            'nonce' => esc_html__('Security validation failed', 'bedentist'),
            'mail' => esc_html__('Please, provide valid mail', 'bedentist'),
            'mailchimp' => esc_html__('Please, set up MailChimp API key and List ID', 'bedentist'),
            'internal' => esc_html__('Internal error. Please, try again later', 'bedentist'),
        );
    ...
    public function widget($args, $instance)
    {
        ...
        if ($subscribe_enabled && $subscribe_template && $api_key && $list_id) {
            ...
            esc_html_e('Please set up MailChimp API key and List ID', 'bedentist');
        }
    ...
    {
        return bedentist_get_social_list('widget');
    }
    ...
    public function get_subscribe_submit($class)
    {
        ...
        $subscribe_submit = bedentist_render_icons($subscribe_submit);
    ...
    public function process_subscribe()
    {
        ...
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'bedentist_subscribe')) {
...
add_action('widgets_init', 'bedentist_register_subscribe_follow_widgets');
function bedentist_register_subscribe_follow_widgets()
{
