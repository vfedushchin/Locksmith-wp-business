 ...
 * @package Travelop
 */
if ( ! class_exists( 'Travelop_About_Author_Widget' ) ) {

	class Travelop_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'travelop widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'travelop' );
			$this->widget_id          = 'travelop_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'travelop' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'travelop' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'travelop' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'travelop' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'travelop' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'travelop' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'travelop' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'travelop' ),
					'label' => esc_html__( 'Link label', 'travelop' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => __( 'Select a user', 'travelop' ) );
...
add_action( 'widgets_init', 'travelop_register_about_author_widgets' );
function travelop_register_about_author_widgets() {
	register_widget( 'Travelop_About_Author_Widget' );
}
