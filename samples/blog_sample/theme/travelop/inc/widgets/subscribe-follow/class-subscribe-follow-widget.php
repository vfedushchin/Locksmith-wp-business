...
class Travelop_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'travelop widget-subscribe';
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'travelop' );
		$this->widget_id          = 'travelop_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'travelop' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'travelop' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'travelop' ),
				'label' => esc_html__( 'Subscribe Title', 'travelop' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'travelop' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'travelop' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'travelop' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'travelop' ),
				'label' => esc_html__( 'Subscribe submit label', 'travelop' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'travelop' ),
				'label' => esc_html__( 'Subscribe success', 'travelop' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'travelop' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'travelop' ),
				'label' => esc_html__( 'Follow Title', 'travelop' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'travelop' ),
			),
		);

		add_action( 'wp_ajax_travelop_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_travelop_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'travelop' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'travelop' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'travelop' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'travelop' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'travelop' );
		}
	...
	public function get_social_nav() {
		return travelop_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = travelop_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'travelop_subscribe' ) ) {
...
add_action( 'widgets_init', 'travelop_register_subscribe_follow_widgets' );
function travelop_register_subscribe_follow_widgets() {
	register_widget( 'Travelop_Subscribe_Follow_Widget' );
}
